...
function openNavTree() {
	$(".web-help-nav li").removeClass("selected");
	if (location.hash.indexOf('q=') == -1 && location.hash.length > 1) {
		...
		//var el1 = $('#web-help-c1 span[href*="'+href+'"');
		//alert($(el1).text);
		...
		setTimeout(function() {
			...
			$(el).scrollintoview({duration:200});
			//$("#web-help-c1").animate({
		...
		treeText = [];
		treeUrl = [];
		child = $("span[href='"+href+"']", $(defaultNav))[0];
		el1 = $(child).parent()[0];
		while (treeComplete != 'true') {

			//treeText.unshift($(el1).html());
			treeText.unshift($(el1).text());
			treeUrl.unshift($(el1).find('span').attr('href'));
			if (el1.parentElement.parentElement != null) {
				el1 = el1.parentElement.parentElement.previousElementSibling;
			} else {treeComplete = 'true'}
		}

		for (t in treeText) {
			if (t == 0) {
				breadString += "<span class='breadLinks'>Home / <a href='#" + treeUrl[t] + "'>" + treeText[t] + "</a>";
			} else if (t != treeText.length - 1) {
				breadString += " / " + "<a href='#" + treeUrl[t] + "'>" + treeText[t] + "</a>";
			} else {
				breadString += " / "  + "<a href='#" + treeUrl[t] + "'>" + treeText[t] + "</a></span>";
			};
		};
		 


			$('#breads').html(breadString);
...
$( document ).ready(function() {
	...
	/* $(document).on("click","#web-help-c2 a",function (e) {
		var href = getPathAfterUpFolder(location.hash.substring(1));
		var el = $('#web-help-c1 span[href*="'+href+'"').parent();
		$("#web-help-c1").animate({
		scrollTop: ($(el).offset().top - ($(window).height()/2))
